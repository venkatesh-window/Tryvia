...
async function runTest() {
  ...
  try {
    ...
    if (vendorA.role !== "VENDOR")
      throw new Error("Vendor A role is not VENDOR");
    ...
    if (vendorB.role !== "VENDOR")
      throw new Error("Vendor B role is not VENDOR");
    ...
    if (customer.role !== "CUSTOMER")
      throw new Error("Customer role is not CUSTOMER");
    ...
    if (!sampleProduct)
      throw new Error("No existing products to copy brand/category from.");
    ...
    const createProduct = async (token, name) => {
    ...
    if (
      !productA.imageUrl ||
      !productA.imageUrl.includes("res.cloudinary.com")
    ) {
      console.warn(
        "⚠️ Product A image URL may not be Cloudinary if env vars are missing:",
        productA.imageUrl,
      );
    }
    if (!productA.vendor) throw new Error("Product A missing vendor reference");
    console.log(
      "✅ Vendor A created product with Cloudinary fallback image upload.",
    );
    ...
    const foundA = feed.find((p) => p.id === productA.id);
    const foundB = feed.find((p) => p.id === productB.id);
    if (!foundA || !foundB)
      throw new Error(
        "Newly created products did not appear in customer feed!",
      );
    console.log(
      "✅ Customer feed contains the new MongoDB products. No mock data.",
    );
    ...
    if (customerOrder.total_amount !== 200) {
      throw new Error(
        `Backend trusted frontend prices! Total should be 200, got ${customerOrder.total_amount}`,
      );
    }
    console.log(
      "✅ Backend strictly enforces real DB pricing. Total:",
      customerOrder.total_amount,
    );
    // Check vendorStatuses slice
    if (
      !customerOrder.vendor_statuses ||
      customerOrder.vendor_statuses.length !== 2
    ) {
      throw new Error("Backend did not segregate vendor statuses correctly!");
    }
    console.log(
      "✅ Order correctly split into multiple vendor fulfillment statuses.",
    );
    ...
    if (vendorAOrders.length < 1) {
      ...
      throw new Error(
        `Vendor A order fetch failed. Expected at least 1 order, got ${vendorAOrders.length}`,
      );
    }
    const recentVendorAOrder = vendorAOrders[0];
    if (recentVendorAOrder.total !== 150)
      throw new Error(
        "Vendor A total is incorrect (should only see their slice)",
      );
    ...
    await axios.patch(
      `${API_BASE}/vendor/orders/${orderId}/status`,
      { status: "SHIPPED" },
      {
    ...
    if (vendorAOrdersRes2.data[0].status === "SHIPPED") {
      throw new Error(
        "Vendor B updating status maliciously affected Vendor A!",
      );
    }
    console.log(
      "✅ Vendor B cannot alter Vendor A's status slice. Cross-vendor update rejected/isolated.",
    );
    ...
    console.log(
      "✅ Vendor A successfully updated their order slice to SHIPPED.",
    );
    ...
    const fetchedCustomerOrder = customerOrders.find((o) => o._id === orderId);
    if (!fetchedCustomerOrder)
      throw new Error("Customer cannot see their order");
    console.log(
      "✅ Customer fetching order is successful. Statuses:",
      fetchedCustomerOrder.vendor_statuses,
    );
