...
import { authenticate, AuthRequest, requireVendor } from '../middleware/auth';
...
router.post('/apply', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (existingVendor) {
      res.status(400).json({ detail: 'You have already applied to be a vendor.' });
      return;
    }
    
    const { storeName, description, email, phone, address, slug, gst, pan } = req.body;
    ...
    res.json({ detail: 'Application submitted successfully', vendor: vendor.toJSON() });
  } catch (error: any) {
    if (error.code === 11000) {
...
const getVendor = async (req: AuthRequest, res: Response, next: any) => {
  try {
    ...
    if (!vendor) {
      res.status(404).json({ detail: 'Vendor account not found for this user.' });
      return;
    }
...
router.get('/application', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    res.json((req as any).vendor.toJSON());
  } catch (error: any) {
    res.status(500).json({ detail: error.message });
  }
});
...
router.get('/profile', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    res.json((req as any).vendor.toJSON());
  } catch (error: any) {
    res.status(500).json({ detail: error.message });
...
router.put('/profile', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    const { storeName, description, phone, address, logo, banner, slug, gst, pan } = req.body;
...
router.get('/dashboard', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    ...
    const lowStockProducts = products.filter(p => p.stockFull < 10 || p.stockTester < 10).length;
    ...
    for (const order of orders) {
      ...
      if (orderHasVendorProducts) {
        ...
        const vStatus = order.vendorStatuses?.find(vs => vs.vendor.equals(vendor._id));
...
router.get('/earnings', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
...
router.get('/analytics', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    
    const products = await Product.find({ vendor: vendor._id }, '_id name stockFull stockTester');
    const productIds = products.map(p => p._id);
    
    const orders = await Order.find({ 'items.product': { $in: productIds } }).populate('items.product', 'name');
    ...
    const lowStockItems = products.filter(p => p.stockFull < 10 || p.stockTester < 10).map(p => ({
      name: p.name,
...
router.get('/products', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    const products = await Product.find({ vendor: vendor._id }).populate('category').populate('brand');
...
router.get('/products/:id', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    const product = await Product.findOne({ _id: req.params.id, vendor: vendor._id }).populate('category').populate('brand');
    if (!product) {
...
router.put('/products/:id', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
...
router.get('/orders', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    ...
    const formattedOrders = orders.map(order => {
      ...
      return {
        id: order.numericId,
        _id: order._id,
        customer: (order.user as any)?.fullName || 'Customer',
...
router.get('/orders/:id', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    ...
    .populate('items.product', 'name imageUrl numericId fullPrice testerPrice');
...
router.patch('/orders/:id/status', getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    ...
    let vStatusIndex = order.vendorStatuses.findIndex(vs => vs.vendor.equals(vendor._id));
    ...
    if (vStatusIndex === -1) {
      ...
      order.vendorStatuses[vStatusIndex].history.push({ status, changedAt: new Date() });
      if (trackingNumber) order.vendorStatuses[vStatusIndex].trackingNumber = trackingNumber;
      if (shippingPartner) order.vendorStatuses[vStatusIndex].shippingPartner = shippingPartner;
    }
    ...
    if (status === 'DELIVERED' && oldStatus !== 'DELIVERED') {
      const vendorItems = order.items.filter(i => productIds.some(id => id.equals(i.product as any)));
