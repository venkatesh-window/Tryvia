...
router.patch('/vendor/:id/status', authenticate, getVendor, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const vendor = (req as any).vendor;
    ...
    const ret = await Return.findOne({ _id: req.params.id, vendor: vendor._id });
    if (!ret) {
    ...
    if (ret.status !== 'REQUESTED') {
      res.status(400).json({ detail: 'Return is no longer in REQUESTED state' });
      return;
    }

    ret.status = status as any;
    ret.history.push({
