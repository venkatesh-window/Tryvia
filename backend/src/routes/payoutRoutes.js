...
export const getVendorBalances = async (vendorId) => {
  ...
  for (const entry of ledgerEntries) {
    if (entry.type === "SALE") {
      ...
      if (["PENDING", "AVAILABLE", "COMPLETED"].includes(entry.status)) {
        // PENDING means requested here, AVAILABLE is processing.
        availableBalance += entry.amount; // amount is negative
      }
...
router.get("/vendor", authenticate, getVendor, async (req, res) => {
  try {
    ...
    const payouts = await Payout.find({ vendor: vendor._id }).sort({
      createdAt: -1,
    });
    const balances = await getVendorBalances(vendor._id);
    // Also get ledger entries for full history
    const ledger = await VendorLedger.find({ vendor: vendor._id })
      .sort({ createdAt: -1 })
      .limit(50);
    res.json({
      balances,
...
router.post("/request", authenticate, getVendor, async (req, res) => {
  ...
  try {
    ...
    if (!vendor.payoutAccount || !vendor.payoutAccount.accountNumber) {
      res
        ...
        .json({
          detail:
            "Please set up your payout account details in store settings first.",
        });
      await session.abortTransaction();
      session.endSession();
      return;
    ...
    if (amount > balances.availableBalance) {
      res
        .status(400)
        .json({
          detail: `Requested amount exceeds available balance of $${balances.availableBalance.toFixed(2)}`,
        });
      await session.abortTransaction();
      session.endSession();
      return;
    ...
    if (existingPending) {
      res
        ...
        .json({
          detail:
            "You already have a payout request in progress. Please wait for it to complete.",
        });
      await session.abortTransaction();
      session.endSession();
      return;
    ...
    try {
      ...
      if (ledgerEntry) {
        if (status === "COMPLETED") {
          ...
          if (paymentReference)
            ledgerEntry.notes = `Completed: ${paymentReference}`;
          await ledgerEntry.save({ session });
          // Notify vendor
