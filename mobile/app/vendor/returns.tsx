  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <Typography style={styles.subtitle}>Manage customer return requests.</Typography>
      </View>

      <View style={styles.listCard}>
        {returns.length === 0 ? (
          <View style={styles.emptyState}>
            <ArrowLeftRight size={40} color="#ECE7E1" style={{ marginBottom: 16 }} />
            <Typography style={styles.emptyText}>No returns requested.</Typography>
          </View>
        ) : (
          returns.map((ret: any, idx: number) => (
            <View key={idx} style={styles.returnCard}>
              <View style={styles.returnHeader}>
                <View>
                  <Typography style={styles.returnId}>Return #{ret._id.toString().substring(0,8)}</Typography>
                  <Typography style={styles.returnDate}>Order #{ret.order?.numericId} • {new Date(ret.createdAt).toLocaleDateString()}</Typography>
                </View>
                <View style={[
                  styles.statusBadge,
                  ret.status === 'APPROVED' || ret.status === 'REFUNDED' ? styles.statusApproved :
                  ret.status === 'REJECTED' || ret.status === 'CANCELLED' ? styles.statusRejected :
                  styles.statusPending
                ]}>
                  <Typography style={[
                    styles.statusText,
                    ret.status === 'APPROVED' || ret.status === 'REFUNDED' ? styles.statusTextApproved :
                    ret.status === 'REJECTED' || ret.status === 'CANCELLED' ? styles.statusTextRejected :
              ...
              <View style={styles.productRow}>
                <Image source={{ uri: ret.product?.imageUrl }} style={styles.productImg} />
                <View style={{ flex: 1 }}>
                  <Typography style={styles.productName} numberOfLines={1}>{ret.product?.name}</Typography>
                  <Typography style={styles.productType}>
                    {ret.quantity}x {ret.itemType === 'full' ? 'Full Size' : 'Tester'}
                  </Typography>
                  <Typography style={styles.reasonText}>
                    <Typography style={{ fontFamily: 'Inter_600SemiBold', color: '#1A1918' }}>Reason: </Typography>
                    {ret.customerReason}
                  </Typography>
                </View>
              </View>
              ...
              {ret.status === 'REQUESTED' && (
                <View style={styles.actionsRow}>
                  ...
                  >
                    {actioning === ret._id ? <ActivityIndicator size="small" color="#318C59" /> : (
                      <>
                        <Check size={16} color="#318C59" />
                        <Typography style={[styles.actionBtnText, { color: '#318C59' }]}>Approve</Typography>
                      </>
                    )}
                  </TouchableOpacity>
                  ...
                  >
                    ...
                    <Typography style={[styles.actionBtnText, { color: '#D9383A' }]}>Reject</Typography>
                  </TouchableOpacity>
                </View>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  productImg: { width: 60, height: 60, borderRadius: 8, backgroundColor: '#F8F6F3' },
  productName: { fontFamily: 'Inter_600SemiBold', fontSize: 14, color: '#1A1918', marginBottom: 4 },
  productType: { fontFamily: 'Inter_500Medium', fontSize: 12, color: '#8E8A85', marginBottom: 8 },
  reasonText: { fontFamily: 'Inter_400Regular', fontSize: 13, color: '#4A4846', fontStyle: 'italic', backgroundColor: '#FAF8F5', padding: 8, borderRadius: 8 },
  actionsRow: { flexDirection: 'row', gap: 12, marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#F8F6F3' },
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 10, borderRadius: 8, borderWidth: 1, gap: 8 },
