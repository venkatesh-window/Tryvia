...
export default function AuthScreen() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      const result = await ClerkService.signIn(email.trim(), password.trim());
      ...
      await setAuthUser(result.token || 'clerk_token', {
        id: 1,
        email: email.trim(),
        fullName: result.user?.fullName || email.split('@')[0] || 'Venkatesh S',
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (result.status === 'failed') {
        setError(result.error || 'Google authentication was cancelled or failed.');
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const result = await ClerkService.verifyCode(signUpId, verificationCode.trim());
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(500).delay(100)} style={styles.switcherTrack}>
          {/* Animated Sliding Pill */}
          <Animated.View style={[styles.slidingPill, { width: tabWidth }, slidingPillStyle]} />
          ...
          >
            <Typography
              style={[styles.switchText, mode === 'signin' && styles.switchTextActive]}
            >
              Sign In
            </Typography>
          </TouchableOpacity>
          ...
          >
            <Typography
              style={[styles.switchText, mode === 'signup' && styles.switchTextActive]}
            >
              Create Account
            </Typography>
          </TouchableOpacity>
        ...
        <View style={styles.formCard}>
          ...
          <Animated.View style={[styles.inputGroup, signupFieldStyle]}>
            ...
            <View style={styles.inputWrapper}>
              <UserIcon size={18} color="#8E8A85" strokeWidth={1.75} style={styles.inputIcon} />
              <TextInput
                style={styles.input}
            ...
            {loading ? (
              ...
              <>
                ...
                <Typography style={styles.submitBtnText}>
                  {mode === 'signin' ? 'Sign In to TryVia' : 'Create Member Account'}
                </Typography>
              </>
            )}
          </TouchableOpacity>
        ...
        <Animated.View entering={FadeInUp.duration(400).delay(200)} style={styles.benefitsCard}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              ...
              <Typography style={styles.modalTitle}>Verify Your Email</Typography>
            ...
            >
              {verifyLoading ? (
                ...
                <Typography style={styles.submitBtnText}>Verify & Continue</Typography>
              )}
            </TouchableOpacity>
          </View>
        </View>
