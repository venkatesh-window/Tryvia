...
import { useOrderStore, OrderStatus, Order } from '../../src/store/useOrderStore';
...
export default function OrdersScreen() {
  ...
  const getStatusDisplay = (order: Order) => {
    ...
    if (order.status === 'SHIPPED') {
      return {
        ...
        bg: '#EFF6FF',
        border: '#DBEAFE',
        note: isCod ? `Pay ₹${order.total.toFixed(2)} on arrival` : 'Courier out for delivery',
      };
    }
  ...
  return (
    <ScreenContainer>
      <View style={[styles.container, { paddingTop: insets.top > 0 ? insets.top + 8 : 18 }]}>
        {/* Top Header */}
          ...
          {orders.length === 0 ? (
            ...
            <Animated.View entering={FadeInUp.duration(500)} style={styles.emptyStateCard}>
              ...
              <Typography style={styles.emptySubtitle}>
                Your order journey begins here. Discover authentic luxury discovery testers and flagship formulations.
              </Typography>
              <TouchableOpacity
              ...
              >
                <Typography style={styles.emptyButtonText}>Explore Formulations</Typography>
              </TouchableOpacity>
            </Animated.View>
          ) : (
                ...
                >
                  ...
                  <View style={styles.orderHeader}>
                    <View style={styles.orderIdentityRow}>
                      <View style={[styles.orderIconBadge, order.status === 'DELIVERED' && styles.deliveredIconBadge]}>
                        <Package size={18} color={order.status === 'DELIVERED' ? '#16A34A' : '#1A1918'} strokeWidth={1.5} />
                      </View>

                      <View style={styles.orderIdentityText}>
                        <Typography style={styles.orderId}>#{order.orderNumber}</Typography>
                    ...
                    <View style={[styles.statusBadge, { backgroundColor: statusInfo.bg, borderColor: statusInfo.border }]}>
                      ...
                      <Typography style={[styles.statusText, { color: statusInfo.color }]}>
                        {statusInfo.label}
                      </Typography>
                    </View>
                  </View>
                  ...
                  {/* Payment Note Banner */}
                  <View style={[styles.paymentNoteBanner, isCod ? styles.codBanner : styles.prepaidBanner]}>
                    {isCod ? (
                      <Banknote size={14} color="#D97706" />
                    ) : (
                      <CreditCard size={14} color="#16A34A" />
                    )}
                    <Typography style={[styles.paymentNoteText, { color: isCod ? '#92400E' : '#166534' }]}>
                      {statusInfo.note}
                    </Typography>
                  </View>
                      ...
                      return (
                        <React.Fragment key={step}>
                          <View style={styles.stepPointWrapper}>
                            ...
                            >
                              {step === 'PENDING' ? (isCod ? 'BOOKED' : 'PLACED') : step}
                            </Typography>
                          </View>
                    ...
                    {order.items.map((item, idx) => (
                      <View key={idx} style={styles.itemRow}>
                        <View style={styles.itemImageWrapper}>
                          <Image
                            source={{ uri: item.imageUrl || 'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?q=80&w=200' }}
                        ...
                        <View style={styles.itemDetails}>
                          <Typography style={styles.itemBrand}>{item.brand.toUpperCase()}</Typography>
                          <Typography style={styles.itemName} numberOfLines={2}>
                            {item.name}
                          </Typography>
                          <Typography style={styles.itemType}>
                            {item.type === 'tester' ? 'Discovery Tester' : 'Flagship Full Size'} • Qty {item.quantity}
                          </Typography>
                        </View>
                  ...
                  >
                    <View style={styles.addressHeaderRow}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, flex: 1 }}>
                        ...
                        <Typography style={styles.addressTitle}>DELIVERY DESTINATION</Typography>
                      </View>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                        <Typography style={styles.addressTapHint}>View Details</Typography>
                  ...
                  <View style={styles.footerContainer}>
                    ...
                    <View style={styles.actionButtonsRow}>
                      ...
                      >
                        ...
                        <Typography style={styles.reorderBtnText}>Reorder</Typography>
                      </TouchableOpacity>
                      ...
                      >
                        ...
                        <Typography style={styles.invoiceBtnText}>Invoice</Typography>
                      </TouchableOpacity>
                      ...
                      >
                        ...
                        <Typography style={styles.reviewBtnText}>Review</Typography>
                      </TouchableOpacity>
                    </View>
                  </View>
                </Animated.View>
        ...
        <AddressDetailModal
          ...
          orderNumber={selectedAddressOrder?.orderNumber ? `#${selectedAddressOrder.orderNumber}` : undefined}
