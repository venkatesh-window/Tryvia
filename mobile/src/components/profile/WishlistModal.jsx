...
import { X, Heart, ShoppingBag, Trash2, Sparkles } from "lucide-react-native";
import * as Haptics from "expo-haptics";
import { theme } from "../../theme/theme";
import { useSafeAreaInsets } from "react-native-safe-area-context";

export const WishlistModal = ({ visible, onClose, onOpenCart }) => {
  const insets = useSafeAreaInsets();
  const { items, removeFromWishlist } = useWishlistStore();
  const { addItem } = useCartStore();

  const handleMoveToBag = (product, type) => {
    if (Platform.OS === "ios") {
          ...
          >
            {items.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                >
                  Tap the heart icon on any product to save it to your bespoke
                  luxury collection.
                </Typography>
                <PremiumButton
