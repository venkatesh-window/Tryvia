...
export const RazorpayCheckoutModal: React.FC<RazorpayCheckoutModalProps> = ({
  ...
  const formatFinalOrder = (backendOrder: any, fallbackMethod: string): Order => {
    const formattedDate = new Intl.DateTimeFormat('en-US', {
      month: 'short',
      day: '2-digit',
      year: 'numeric',
    }).format(new Date(backendOrder.created_at || backendOrder.createdAt || Date.now()));

    return {
      id: backendOrder.id ? backendOrder.id.toString() : String(Date.now()).slice(-6),
      orderNumber: `TRV-${10000 + (Number(backendOrder.numericId || backendOrder.id) || 1)}`,
      date: formattedDate,
      subtotal: backendOrder.subtotal ?? subtotal,
      walletDeduction: backendOrder.walletDiscount ?? backendOrder.wallet_discount ?? walletDeduction,
      ...
      status: (backendOrder.status as OrderStatus) || 'PAID',
      paymentMethod: backendOrder.paymentMethod || backendOrder.payment_method || fallbackMethod,
      shippingAddress: backendOrder.shippingAddress || backendOrder.shipping_address || deliveryAddress,
      items: items.map((i) => ({
        id: String(i.id),
        name: i.product.name,
  ...
  const handleOpenStandardRazorpay = async () => {
    ...
    try {
      ...
      if (rzpOrder.is_zero_amount && rzpOrder.order) {
        ...
        useOrderStore.setState((state) => ({ orders: [formatted, ...state.orders] }));
        clearCart();
      ...
      if (Platform.OS === 'web' && typeof window !== 'undefined' && (window as any).Razorpay) {
        const options = {
          ...
          handler: async (response: any) => {
            setProcessingStep('Verifying cryptographic signature with Razorpay...');
            try {
              ...
              const formatted = formatFinalOrder(verifyRes.order, 'Razorpay Checkout');
              useOrderStore.setState((state) => ({ orders: [formatted, ...state.orders] }));
              clearCart();
              setIsProcessing(false);
              onSuccess(formatted);
            } catch (err: any) {
              console.error('Payment signature verification failed:', err?.response?.data || err?.message);
              setIsProcessing(false);
              const errMsg = err?.response?.data?.detail || err?.message || 'Payment signature could not be verified.';
      ...
      const sessionUrl = paymentService.getCheckoutSessionUrl(rzpOrder.order_id);
      ...
      if (result.type === 'success' && result.url) {
        ...
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, status, error: sessionErr } = parsed.queryParams || {};
        ...
        if (status === 'failed' || sessionErr) {
          setIsProcessing(false);
          Alert.alert('Payment Failed', (sessionErr as string) || 'Payment transaction was declined.');
          return;
        }
        ...
        if (razorpay_payment_id && razorpay_signature) {
          ...
          try {
            const verifyRes = await paymentService.verifyPayment({
              razorpay_order_id: (razorpay_order_id as string) || rzpOrder.razorpay_order_id || '',
            ...
            const formatted = formatFinalOrder(verifyRes.order, 'Razorpay Mobile');
            useOrderStore.setState((state) => ({ orders: [formatted, ...state.orders] }));
            clearCart();
            setIsProcessing(false);
            onSuccess(formatted);
          } catch (err: any) {
            console.error('Mobile payment verification failed:', err?.response?.data || err?.message);
            setIsProcessing(false);
            const errMsg = err?.response?.data?.detail || err?.message || 'Payment signature could not be verified.';
            Alert.alert('Verification Failed', errMsg);
          }
      ...
      setIsProcessing(false);
      const errMsg = error?.response?.data?.detail || error?.message || 'Unable to complete Razorpay payment.';
      ...
      >
        ...
        <View style={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 24) }]}>
          ...
          <View style={styles.amountBar}>
            <View>
              <Typography variant="caption" color="secondary" style={{ fontSize: 11, letterSpacing: 1 }}>
                TOTAL PAYABLE
              </Typography>
          ...
          {isProcessing ? (
            <View style={styles.processingContainer}>
              ...
              <Typography
                variant="body"
                weight="medium"
                style={{ color: theme.colors.text.primary, marginBottom: 8, fontSize: 16 }}
              >
            ...
            >
              ...
              {deliveryAddress && (
                <View style={styles.addressPill}>
                  ...
                  <Typography variant="caption" color="secondary" numberOfLines={1} style={styles.addressText}>
              ...
              <View style={styles.razorpayCard}>
                <View style={styles.gatewayHeader}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    ...
                    <View>
                      <Typography variant="body" weight="bold" style={{ color: '#0C2340', fontSize: 15 }}>
                        Razorpay Gateway
                      </Typography>
                      <Typography variant="caption" color="secondary" style={{ fontSize: 11.5 }}>
                ...
                <Typography variant="caption" color="secondary" style={styles.instrumentsLabel}>
                  SUPPORTED PAYMENT METHODS:
                </Typography>
                <View style={styles.instrumentsGrid}>
                  <View style={styles.instrumentBadge}>
                    <Smartphone size={13} color="#1A1918" />
                    <Typography style={styles.instrumentText}>UPI (GPay / PhonePe / Paytm / BHIM)</Typography>
                  </View>
                  <View style={styles.instrumentBadge}>
                    <CreditCard size={13} color="#1A1918" />
                    <Typography style={styles.instrumentText}>Cards (Credit / Debit / EMI)</Typography>
                  </View>
                  <View style={styles.instrumentBadge}>
                    <Building2 size={13} color="#1A1918" />
                    <Typography style={styles.instrumentText}>NetBanking (50+ Indian Banks)</Typography>
                  </View>
                  <View style={styles.instrumentBadge}>
                    <Wallet size={13} color="#1A1918" />
                    <Typography style={styles.instrumentText}>Wallets & PayLater</Typography>
                  </View>
                </View>
                ...
                <View style={styles.featureGrid}>
                  <View style={styles.featureItem}>
                    ...
                    <Typography style={styles.featureText}>256-bit SSL Encryption</Typography>
                  </View>
                  <View style={styles.featureItem}>
                    <CheckCircle2 size={13} color="#16A34A" />
                    <Typography style={styles.featureText}>PCI-DSS Level 1 Certified</Typography>
                  </View>
                  <View style={styles.featureItem}>
                    <ShieldCheck size={13} color="#16A34A" />
                    <Typography style={styles.featureText}>Instant Refund Protection</Typography>
                  </View>
                </View>
              </View>
